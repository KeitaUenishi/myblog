...
git worktree add ../myblog-{team}-{x} -b {team}-{x}-base
mkdir -p ../myblog-{team}-{x}/.claude
cp .claude/settings.local.json ../myblog-{team}-{x}/.claude/

# 依存インストール（バックグラウンドで並列実行）
cd ../myblog-{team}-{x} && npm install --frozen-lockfile
```
...
| Biomeフォーマットエラー | worktreeで `npm run biome:check:write` して再push |
...
# worktree削除
git worktree remove ../myblog-{team}-{x}
...
### worktreeの注意
- パスは `../myblog-{name}` 形式（CLAUDE.mdのルール）
- ブランチ名は一意にすること（`{team}-{x}-base` パターン推奨）
- `npm install` は並列実行しても安全（lockfileが同じため）
