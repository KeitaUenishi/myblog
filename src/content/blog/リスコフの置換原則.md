...
> 3. 不変条件（invariants）は、派生型でも保護されねばならない。派生型でそのまま維持される。
...
OKの例は以下の通り。`TonerEmptyException` や `PaperJamException`は`PrinterException`の派生のため、呼び出し側は `PrinterException` として例外を処理することが可能になる。このように例外においても子クラスは親の契約に準じた形で、置換可能であることが重要である。
```java
...
> 1番信頼できない入力が来るレイヤーである、ユーザーからの入力を受け入れるところでは防御的プログラミングで書き、それ以降のレイヤーは防御的プログラミングにする必要がない。
> 
